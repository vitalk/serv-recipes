...
ln -s /etc/nginx/sites-{avaliable,enabled}/$my_app

sed -i 's/YOUR_APP_NAME/$my_app/' /etc/nginx/sites-avaliable/$my_app
sed -i 's/YOUR_SERVER_IP:80/80/' /etc/nginx/sites-avaliable/$my_app
sed -i 's/YOUR_SERVER_FQDN/$my_domain/' /etc/nginx/sites-avaliable/$my_app
